...
//                  readings and the mean value. [A quick check to see that it
//                  is working is to connect power or ground (on the same header)
//                  to AN2 with a wire. You should see a reading of 0 for ground
//                  and 1023 for 3.3V.]
...
volatile int L_R_value, U_D_value;
...
void __ISR(_ADC_VECTOR, IPL7SRS) _ADCHandler(void) {
    ...
    if (current_reading == NUM_ADC_SAMPLES) {
        ...
        L_R_value = LRtotal >> LOG2_NUM_ADC_SAMPLES; // divide by num of samples
        U_D_value = UDtotal >> LOG2_NUM_ADC_SAMPLES; // divide by num of samples
    }
    DBG_OFF(MASK_DBG1);
...
void initTimer3() 
{
    ...
    // The period of Timer 3 is (16 * 625)/(10 MHz) = 1ms.
...
int main() 
{
    ...
    // turn pin 4 of Port A for output     
    TRISBCLR = 0x0040;   
    ODCBCLR  = 0x0040;   
    LATBSET = 0x0040;
...
//    OledUpdate();
    ...
    while (1) 
    {
        ...
        if (ms_since_last_oled_update >= NUMBER_OF_MILLISECONDS_PER_OLED_UPDATE) 
        {
            ...
            sprintf(oledstring, "%d", timer2_ms_value_save / 100);
            OledSetCursor(12, 0);
            OledPutString(oledstring);
            sprintf(oledstring, "L/R = %4d", L_R_value);
            OledSetCursor(0, 0);
            OledPutString(oledstring);
            sprintf(oledstring, "U/D = %4d", U_D_value);
            OledSetCursor(0, 1);
            OledPutString(oledstring);
            if (L_R_value<411)
            {
