...
OledGetBmp(int dxco, int dyco, BYTE * pbBits)
	...
	*/
	...
	if (xcoRight >= ccolOledMax) {
		//xcoRight = ccolOledMax - 1;   // ALA BUG
        xcoRight = ccolOledMax;         // ALA fixed
	}

	ycoTop = ycoOledCur;
	ycoBottom = ycoTop + dyco;
	if (ycoBottom >= crowOledMax) {
		//ycoBottom = crowOledMax - 1;  // ALA BUG
        ycoBottom = crowOledMax;        // ALA fixed
	}
